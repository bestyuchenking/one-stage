...
function cleanTitle(raw=''){
  ...
  return text.replace(/\s*[|｜]\s*(知乎|豆瓣电影|豆瓣)\s*$/i,'').replace(/\s*-\s*(知乎|豆瓣电影|豆瓣)\s*$/i,'').replace(/^知乎[：:]\s*/,'').trim()
}
...
const film=[]
for(const [href,fallback] of sources.film){ const zh=await fetchTitle(href,fallback); film.push({zh,en:await translate(zh),href,meta:'Douban · Film Review'}) }
const answers=[]
for(const [href,fallback] of sources.answer){ const zh=await fetchTitle(href,fallback); answers.push({zh,en:await translate(zh),href}) }
...
const entertainment=`function Entertainment({lang}:{lang:Lang}){
  ...
  const albums=[{zh:'牛马假日',en:'NiúMǎ Holiday',id:'385484900'},{zh:'池塘里的那片海',en:'The Sea in the Pond',id:'385484965'}]
