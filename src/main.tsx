...
function Entertainment({lang}:{lang:Lang}) {
  return <main className="page entertainment-page">
    ...
    <section className="show-intro"><div><p className="eyebrow">ORIGINAL PODCAST</p><h2>牛马假日</h2><p className="show-desc">{lang==='en'?'A podcast documenting the lives, work and small absurdities of ordinary working people.':'一档记录普通打工人故事的播客。聊工作，也聊生活，以及那些发生在“牛马”与“假日”之间的真实切片。'}</p></div><div className="show-meta"><span>01 / ONESTAGE ENTERTAINMENT</span><span>{lang==='en'?'AUDIO + VIDEO PODCAST':'音频 + 视频播客'}</span></div></section>
    <section className="media-layout">
      <div className="audio-column"><div className="section-label"><span>{lang==='en'?'LISTEN TO ALL EPISODES':'收听全部节目'}</span><span>RSS</span></div><div className="rss-player"><iframe title="牛马假日 podcast player" src="https://play.prx.org/e?uf=https%3A%2F%2Ffeed.xyzfm.space%2Fr6r6p7dccgbw&sp=all" loading="lazy"/></div><p className="feed-note">{lang==='en'?'Episodes are synced automatically from the podcast RSS feed.':'节目列表通过播客 RSS 自动同步更新。'}</p></div>
      <div className="video-column"><div className="section-label"><span>{lang==='en'?'LATEST VIDEO':'最新视频'}</span><span>{lang==='en'?'YOUTUBE':'BILIBILI'}</span></div><div className="video-placeholder"><Placeholder>{lang==='en'?<>YOUTUBE VIDEO<br/>LINK TO BE ADDED</>:<>BILIBILI VIDEO<br/>LINK TO BE ADDED</>}</Placeholder></div><p className="feed-note">{lang==='en'?'The latest episode video will appear here.':'最新一期视频将显示在这里。'}</p></div>
    </section>
    <section className="platform-row"><span>{lang==='en'?'ALSO AVAILABLE ON':'同步发布于'}</span><span>小宇宙 · 喜马拉雅 · Apple Podcasts · YouTube · Bilibili</span></section>
  </main>
}
