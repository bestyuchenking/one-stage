...
function Entertainment({lang}:{lang:Lang}) {
  ...
  return <main className="page entertainment-page">
    ...
    <section className="media-layout">
      <div className="audio-column"><div className="section-label"><span>{lang==='en'?'LISTEN':'收听'}</span><span>RSS</span></div><div className="rss-player"><iframe title={lang==='en'?'NiúMǎ Holiday audio player':'牛马假日音频播放器'} src="https://play.prx.org/e?uf=https%3A%2F%2Ffeed.xyzfm.space%2Fr6r6p7dccgbw" loading="lazy"/></div><p className="feed-note">{lang==='en'?'Latest episode · Play / Pause':'最新一期 · 开始 / 暂停'}</p></div>
    ...
    <section className="ratings-row"><span>{lang==='en'?'RATINGS':'评分网站'}</span><a href={lang==='en'?'https://www.imdb.com/title/tt44772588':'https://www.douban.com/doubanapp/dispatch/movie/38625203'} target="_blank" rel="noreferrer">{lang==='en'?'IMDb':'豆瓣'} ↗</a></section>
